...
export default function LoginPage() {
  ...
  // Déjà connecté → vers l'espace du rôle.
  useEffect(() => {}, [])
  if (session && profile) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    // En cas de succès, la redirection se fait via le Navigate ci-dessus
    // dès que le profil est chargé.
  }
  ...
  return (
    <div className="flex min-h-screen bg-white dark:bg-garde">
      {/* Colonne formulaire */}
      <div className="flex w-full flex-col px-6 py-8 sm:px-10 lg:w-[46%] lg:px-16">
        ...
        <div className="flex flex-1 flex-col justify-center py-10">
          <div className="mx-auto w-full max-w-sm">
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              Accédez à votre espace de coordination des transferts.
            </p>

            {reason && (
              <div className="mt-5 flex items-start gap-2.5 rounded-xl bg-triage/10 px-3.5 py-3 text-[13px] text-triage">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-7 space-y-4" noValidate>
              ...
              {formError && (
                <div role="alert" className="rounded-xl bg-vital/10 px-3.5 py-3 text-[13px] font-medium text-vital">
                  {formError}
                </div>
              )}
            ...
            <p className="mt-5 text-center text-[13px] text-slate-400">
              Les comptes sont créés par les administrateurs. Pas d'inscription publique.
            </p>

            {/* Comptes de démonstration */}
            <div className="mt-6 rounded-xl border border-slate-200 dark:border-garde-bord">
              <button
                type="button"
                onClick={() => setShowDemo((s) => !s)}
                className="flex w-full items-center justify-between px-4 py-3 text-[13px] font-semibold text-slate-600 dark:text-slate-300"
                aria-expanded={showDemo}
              >
                <span className="inline-flex items-center gap-2">
                  <ShieldCheck className="h-4 w-4 text-bloc-clair" aria-hidden />
                  Comptes de démonstration
                </span>
              ...
              {showDemo && (
                <motion.div
                  ...
                  className="overflow-hidden border-t border-slate-100 dark:border-garde-bord"
                >
                  <ul className="divide-y divide-slate-100 dark:divide-garde-bord">
                    {DEMO_ACCOUNTS.map((a) => (
                      <li key={a.email} className="flex items-center justify-between gap-3 px-4 py-2.5">
                        ...
                        <Button type="button" variant="subtle" size="sm" onClick={() => fillDemo(a.email)}>
                          Remplir
                        </Button>
                      </li>
                    ))}
                  ...
                  <p className="px-4 py-2.5 text-[11px] text-slate-400">
                    Mot de passe commun : <span className="font-semibold">{DEMO_PASSWORD}</span>
                  </p>
                </motion.div>
              )}
            </div>
        ...
        <p className="text-center text-[11px] text-slate-400 lg:text-left">
          Données patient minimales et anonymisées — conforme aux exigences de la CDP.
        </p>
      </div>
