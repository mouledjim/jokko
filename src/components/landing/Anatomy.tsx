...
export function HeartBeat({ className, color = '#e11d48', glow = '#fb7185' }: { className?: string } & Palette) {
  ...
  return (
    <svg viewBox="0 0 140 140" className={className} role="img" aria-label="Cœur qui bat">
      ...
      <circle cx="68" cy="78" r="44" fill={`url(#hglow-${id})`} className={cn(!reduce && 'animate-heart-glow')} style={{ transformOrigin: '68px 78px' }} />
      <g
        fill="none"
        stroke={color}
        strokeWidth="1.7"
...
export function Lungs({ className, color = '#e11d48' }: { className?: string } & Palette) {
  const reduce = useReducedMotion()
  const id = useId().replace(/:/g, '')
  return (
    <svg viewBox="0 0 140 140" className={className} role="img" aria-label="Poumons qui respirent">
      ...
      <g fill="none" stroke={color} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" filter={`url(#lf-${id})`}>
...
export function EcgContinuous({ className, color = '#e11d48' }: { className?: string } & Palette) {
  const reduce = useReducedMotion()
  const id = useId().replace(/:/g, '')
    ...
    <svg viewBox="0 0 240 90" preserveAspectRatio="none" className={className} role="img" aria-label="Tracé ECG continu">
      <defs>
        <pattern id={`grid-${id}`} width="12" height="12" patternUnits="userSpaceOnUse">
          <path d="M12 0 H0 V12" fill="none" stroke={color} strokeWidth="0.4" strokeOpacity="0.15" />
        </pattern>
      ...
      <path d={d} fill="none" stroke={color} strokeWidth="1" strokeOpacity="0.18" />
      <path
        d={d}
        fill="none"
        stroke={color}
      ...
      />
      {!reduce && (
        <circle r="3.2" fill={color} filter={`url(#ef-${id})`}>
